...
void Tracking::MonocularInitialization()
{
    ...
    else    //如果单目初始化器已经被创建
    {
        ...
        // Step 5 通过H模型或F模型进行单目初始化，得到两帧间相对运动、初始MapPoints  （重要）
            ...
            vbTriangulated))    //以及对应于mvIniMatches来讲,其中哪些点被三角化了
        {
            ...
            {
                if(mvIniMatches[i]>=0 && !vbTriangulated[i]) // mvIniMatches[i]>=0 有匹配关系 / !vbTriangulated[i]  没有成功初始化
                {
                    mvIniMatches[i]=-1;
                    nmatches--;
                }
