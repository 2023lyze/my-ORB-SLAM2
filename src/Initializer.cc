 ...
 * Step 2 在所有匹配特征点对中随机选择8对匹配特征点为一组，用于估计H矩阵（单应矩阵）和F矩阵（基础矩阵）
                            ...
                            cv::Mat &R21, cv::Mat &t21, vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated, float minParallax, int minTriangulated)
{
    ...
    // 条件2: 如果存在两组及以上的解能三角化出 >0.7*maxGood的点，说明没有明显最优结果，（相似的太多了，没有区分性）失败
...
/**
 * @brief 用位姿来对特征匹配点三角化，从中筛选中合格的三维点  / 用R,t来对特征匹配点三角化，并根据三角化结果判断R，t的合法性
 * 
 * @param[in] R                                     旋转矩阵R
 * @param[in] t                                     平移矩阵t
...
{   
    ...
    if(nGood>0)
    {
		...
		// 如果大于50个，就取排名第50个的较小的视差角即可，为了避免3D点太多时出现太小的视差角 （比如无穷远点的视差角非常小）
